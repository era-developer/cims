// Nightly database backup. Run by the Windows scheduled task "KIMS Backup"
// (see install-kims-backup-task.ps1); safe to run by hand any time.
//
...
//
// Files land in data/backups/kims-YYYY-MM-DD.db. Copy that folder somewhere
...
//
// Usage: node backup-db.js [--db ../data/kims.db] [--keep-days 30]
...
function parseArgs(argv) {
  const args = { db: path.join(DATA_ROOT, 'kims.db'), keepDays: 30 };
...
function main() {
  ...
  const target = path.join(BACKUPS_DIR, `kims-${today}.db`);
  ...
  // each month for the last 12 months. Manual snapshots (kims-before-*.db)
  ...
  const dailies = fs.readdirSync(BACKUPS_DIR)
    .filter(f => /^kims-\d{4}-\d{2}-\d{2}\.db$/.test(f))
    .sort();
