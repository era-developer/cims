...
export default function AdminTransfers() {
  ...
  // ?q= comes from the navbar search box and from notification links, which
  // carry the transfer id so a tap lands on that one request.
  ...
  const filteredRequests = requests.filter(request => {
    ...
    const haystack = [
      request.id, request.requestingCenterName, request.supplyCenterName, request.requestedBy,
      request.programName, request.responsiblePerson, request.purpose,
      ...(request.components || []).map(component => component.name),
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={{ ...styles.centerRow, ...(isMobile ? styles.centerRowMobile : {}) }}>
        {isSuperAdmin && (
        ...
        <input
          ...
          placeholder="Search by transfer ID, center, component, program or person"
...
const styles = {
  ...
  centerRow: { marginBottom: '16px', display: 'flex', gap: '12px', flexWrap: 'wrap' },
  ...
  searchInput: { flex: 1, minWidth: '240px', padding: '10px 12px', borderRadius: '10px', border: '1.5px solid #dbe3f0', fontSize: '14px', background: '#fff' },
